  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: const CustomAppBar(
        ...
        action: false,
        dsibalBack: true,
        actions: [ImpersonationExitButton()],
      ),
      body: AppPullToRefresh(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            Obx(() {
              controller.tasksFilterEpoch.value;
              final pinned = controller.pinnedPersistentTasks;
            ...
            Obx(() {
              ...
              final pinned = controller.pinnedPersistentTasks;
              if (filtered.isEmpty && pinned.isEmpty) {
                return const SliverFillRemaining(
                    child: Center(child: ShowNoData()));
              }

              final keys =
                  controller.orderedDisplayKeys(filtered.keys.toList());
              final isDaily = controller.tasksViewMode.value ==
                  EmployeeDashbordController.tasksViewDaily;
              final todayKey =
                  EmployeeDashbordController.dateKeyFrom(DateTime.now());
                  ...
                  final tasks = (filtered[dateKey] ?? [])
                      .where((e) => controller.currentTab.value == 0
                          ? isEmployeeTaskActive(e.status) &&
                              !isPinnedPersistentTask(e)
                          : e.status == 'completed')
                      .toList();

                  final isToday = dateKey == todayKey;
                  if (tasks.isEmpty && !isDaily && !isToday) {
