...
enum _SalesLensPhase { sections, shelves }
...
class _SalesLocationFilterFabState extends State<SalesLocationFilterFab> {
  ...
  _SalesLensPhase _phase = _SalesLensPhase.sections;
  ...
  List<dynamic> get _currentPageItems {
    final source = _phase == _SalesLensPhase.sections
        ? _activeSections
        : _shelves;
  ...
  int get _pageCount {
    final total = _phase == _SalesLensPhase.sections
  ...
  Future<void> _openLens() async {
    ...
    _phase = _SalesLensPhase.sections;
    _highlightIndex = -1;
    _segmentPage = 0;
    _pageDragAccum = 0;
  ...
  Future<void> _enterShelfPhase(StoreSectionModel section) async {
    _phase = _SalesLensPhase.shelves;
    ...
    _overlay?.markNeedsBuild();
    try {
      _shelves = await _sales.loadPickerSectionShelves(section.id);
    } catch (_) {
      _shelves = const [];
    } finally {
      if (mounted) {
  ...
  void _commitCurrentSelection() {
    if (_phase == _SalesLensPhase.sections) {
      if (_highlightIndex < 0) {
        ...
        if (sections.isNotEmpty) {
          ...
          _sales.applyPickerLocationFilter(
    ...
    if (_highlightIndex < 0) {
      _sales.applyPickerLocationFilter(sectionId: sectionId);
  ...
  Widget _buildOverlay(BuildContext context) {
    ...
    final totalCount = _phase == _SalesLensPhase.sections
        ? _activeSections.length
        : _shelves.length;
    ...
    return Material(
      ...
      child: Listener(
        ...
        onPointerDown: (_) {
          _pageDragAccum = 0;
          _syncAnchor(context);
          if (_phase == _SalesLensPhase.shelves) {
            _shelfGestureActive = true;
          }
        },
        onPointerMove: (e) {
          if (_phase == _SalesLensPhase.shelves && !_shelfGestureActive) return;
        ...
        child: Stack(
          children: [
                ...
                child: IgnorePointer(
                  child: Center(
                    child: _CenterSelectionBanner(
                      title: bannerLabel,
                      subtitle: _phase == _SalesLensPhase.sections
            ...
            if (anchor != null) ...[
                ...
                ...slots.map((slot) {
                  ...
                  return Positioned(
                    ...
                    child: Transform.scale(
                      ...
                      child: _phase == _SalesLensPhase.sections
                          ? _SectionBubble(
              ...
              if (_phase == _SalesLensPhase.shelves && !_loading)
                Positioned(
                  left: _fabOnLeft ? anchor.dx + 8 : anchor.dx - 168,
