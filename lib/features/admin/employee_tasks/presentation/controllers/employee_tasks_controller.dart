...
class EmployeeTasksController extends GetxController {
  ...
  int get todayRemainingTasksCount {
    if (!_ongoingLoaded) return 0;
    ...
    for (final task in employeeTaskService.ongoingEmployeeTasks.values
      ...
      if (dateKeyFrom(task.startTime) != todayKey) continue;
      keys.add(
  ...
  Map<String, List<EmployeeTaskModel>> _applyNameAndRange(
    ...
    final withName =
        !hasEmployeeFilter && !hasTextFilter ? sorted : filterTasks(sorted);
  ...
  String? _activeTaskDetailsRequestKey;
  Future<void>? _activeTaskDetailsRequest;

  // Coalesce duplicate requests generated by fast taps or route initialization.
  Future<void> getTaskDetails({
    ...
    final requestKey = '$taskId|${occurrenceId ?? ''}|${taskDate ?? ''}';
    final activeRequest = _activeTaskDetailsRequest;
    if (activeRequest != null && _activeTaskDetailsRequestKey == requestKey) {
      return activeRequest;
    }

    late final Future<void> trackedRequest;
    trackedRequest = _loadTaskDetails(
      taskId: taskId,
      occurrenceId: occurrenceId,
      taskDate: taskDate,
      showFullScreenLoader: showFullScreenLoader,
    ).whenComplete(() {
      if (identical(_activeTaskDetailsRequest, trackedRequest)) {
        _activeTaskDetailsRequest = null;
        _activeTaskDetailsRequestKey = null;
      }
    });
    _activeTaskDetailsRequestKey = requestKey;
    _activeTaskDetailsRequest = trackedRequest;
    return trackedRequest;
  }

  Future<void> _loadTaskDetails({
    required String taskId,
    String? occurrenceId,
    String? taskDate,
    ...
    final effectiveTaskDate = (occurrenceId == null || occurrenceId.isEmpty)
  ...
  Future<void> openTaskDetails(EmployeeTaskModel task) async {
    ...
    final occurrenceId =
        actionTask.isOccurrence ? actionTask.occurrenceId?.toString() : null;
    final taskDate =
        occurrenceId == null ? dateKeyFrom(actionTask.startTime) : null;
